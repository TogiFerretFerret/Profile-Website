...
LRU with `maxmemory-samples=5` is probabilistic — recently-accessed keys can still get unlucky. upload 5 copies of each marker and require 3+ to survive. $P(\text{false positive}) \approx 0.15\%$ per bigram while tolerating ~5% per-copy loss.
